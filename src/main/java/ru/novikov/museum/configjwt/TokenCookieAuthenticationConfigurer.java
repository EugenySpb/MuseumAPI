...
public class TokenCookieAuthenticationConfigurer extends AbstractHttpConfigurer<TokenCookieAuthenticationConfigurer, HttpSecurity> {
    ...
    public TokenCookieAuthenticationConfigurer(AuthenticationManager authenticationManager) {
        ...
        builder.logout(logout -> logout.addLogoutHandler(
                ...
                .addLogoutHandler((request, response, authentication) -> {
                    if (authentication != null && authentication.getPrincipal() instanceof TokenUser user) {
                        this.jdbcTemplate.update("insert into deactivated_token (id, c_keep_until) values (?, ?)",
    ...
    public void configure(HttpSecurity builder) throws Exception {
        var cookieAuthenticationFilter = new AuthenticationFilter(authenticationManager,
